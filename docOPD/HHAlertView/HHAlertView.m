...
#define OKBUTTON_BACKGROUND_COLOR [UIColor dOPDThemeColor]
...
- (void)configtext:(NSString *)title detail:(NSString *)detail
{
    ...
    mainFrame.size.height =_detailLabel.size.height + _detailLabel.origin.y+20;
...
- (void)configButton:(NSString *)cancel Okbutton:(NSString *)ok
{
    if (cancel==nil && ok==nil) {
        ...
        [_OkButton setBackgroundColor:[UIColor dOPDThemeColor]];
...
- (void)drawTick
{
   ...
   // layer.strokeColor = [UIColor greenColor].CGColor;//Ashutosh Change
    layer.strokeColor = [UIColor dOPDThemeColor].CGColor;//Ashutosh Change
    layer.lineWidth = 5;
    layer.path = path.CGPath;
...
//    NSLog(@"Enter in HideHHALertview");
    [NSTimer scheduledTimerWithTimeInterval:3.0 target:self selector:@selector(HideView) userInfo:nil repeats:NO];
