...
- (void)viewDidLoad {
    ...
    self.SortBtn.layer.borderWidth=0.4f;
    self.SortBtn.layer.borderColor=[[UIColor colorWithRed:245/255.0f green:245/255.0f blue:245/255.0f alpha:1.0f] CGColor];
    self.SpecializationBtn.layer.borderWidth=0.4f;
    self.SpecializationBtn.layer.borderColor=[[UIColor colorWithRed:245/255.0f green:245/255.0f blue:245/255.0f alpha:1.0f] CGColor];
    ...
    if ([self.ShowDataFor isEqualToString:@"pro"])
    {
        ...
        self.titleLbl.text = [NSString stringWithFormat:@"Search Results: %@",self.ProName];
        ...
        self.titleLbl.text = [NSString stringWithFormat:@"Doctors For: %@",self.ProName];
        [self.tableView addPullToRefreshWithActionHandler:^{
...
-(void)viewWillAppear:(BOOL)animated{
   ...
   // hosptalDataBackUp = [[NSMutableArray alloc]init];

    pgIndex= 1;
    NSString *name = [NSString stringWithFormat:@"OPDScreen"];
    [self.navigationController setNavigationBarHidden:YES];
    id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
    [tracker set:kGAIScreenName value:name];
    [tracker send:[[GAIDictionaryBuilder createScreenView] build]];
    [Localytics tagEvent:@"OPDScreen"];
    BOOL removeLoadFlag=[docOPDNetworkEngine sharedInstance].removeLoadFlag;
    if (removeLoadFlag){
        self.SortLbl.text=@"";
        [self.SortBtn setTitle:@"Sort" forState:UIControlStateNormal];
        hosptalData = [[NSMutableArray alloc]init];
        __weak PhysioVC *weakSelf = self;
        if ([[docOPDNetworkEngine sharedInstance].SpecializationName isEqualToString:@"Specialization"]) {
            self.specializationLbl.text=@"";
            [self.SpecializationBtn setTitle:[docOPDNetworkEngine sharedInstance].SpecializationName forState:UIControlStateNormal];
            [self performSelector:@selector(ApiCallingForSpec) withObject:nil afterDelay:0.000];
            self.titleLbl.text = [NSString stringWithFormat:@"Doctors For: OPD"];
            [self.tableView addPullToRefreshWithActionHandler:^{
                [weakSelf ApiCallingForSpec];
            } position:SVPullToRefreshPositionBottom];
            ...
            [self performSelector:@selector(ApiCallingForSpec) withObject:nil afterDelay:0.000];
            self.titleLbl.text = [NSString stringWithFormat:@"Doctors For: %@",[docOPDNetworkEngine sharedInstance].SpecializationName];
            [self.tableView addPullToRefreshWithActionHandler:^{
                [weakSelf ApiCallingForSpec];
            } position:SVPullToRefreshPositionBottom];
            self.specializationLbl.text=@"Specialization";
            [self.SpecializationBtn setTitle:[docOPDNetworkEngine sharedInstance].SpecializationName forState:UIControlStateNormal];
        }
...
- (IBAction)backAction:(id)sender {
    [Localytics tagEvent:@"OPDHomeClick"];
...
-(void)ApiCallingForSpec{
    ...
    //removeLoadFlag
       ...
       pgIndex>1?:[[AppDelegate MyappDelegate] showIndicator];
        [docOPDNetworkEngine sharedInstance].removeLoadFlag=NO;
    
    self.SortLbl.text=@"";
    [self.SortBtn setTitle:@"Sort" forState:UIControlStateNormal];
    
    NSDictionary *aDic = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:kGetDoctorByNormalSpecialist], keyRequestType,nil];
    //  NSDictionary *dataDic =[NSDictionary dictionaryWithObjectsAndKeys:self.firstName,fName,self.middleName,mName,self.lastName,lName,pgIndex,PageIndex,nil];
    //    NSLog(@"DocData = %@",self.DocData);
    //self.specID
    NSLog(@"%@",[docOPDNetworkEngine sharedInstance].SpecializationID);
    NSDictionary *dataDic =[NSDictionary dictionaryWithObjectsAndKeys:[docOPDNetworkEngine sharedInstance].SpecializationID,@"SpecId",[NSString stringWithFormat:@"%ld",(long)pgIndex],PageIndex,nil];
    
    //    NSLog(@"dataDic for GetDoctorBySpecialist: %@",dataDic);
    server *obj = [[server alloc] init];
    currentRequestType = kGetDoctorByNormalSpecialist;//kGetDoctorByNormalSpecialist
    obj.delegate = self;
    [obj sendRequestToServer:aDic withDataDic:dataDic];
    //    NSLog(@"APICalling for Doctor List");
...
- (void) requestFinished:(NSDictionary * )responseData
{
    ...
    }else if(currentRequestType == kGetDoctorByNormalSpecialist){
...

{
    ...
    if ([[response objectForKey:@"Status"]integerValue]==1)
    {
        if ([hospitalTempData count]==0 || (!hospitalTempData) || hospitalTempData==nil)  {
            ...
            for (int i=0; i<[hospitalTempData count]; i++) {
                NSArray *hospitalArr=[[hospitalTempData objectAtIndex:i]objectForKey:@"HospitalList"];
                NSString *DocFee=[[hospitalArr valueForKey:@"DoctorFee"]objectAtIndex:0];
                NSString *DocDisFee=[[hospitalArr valueForKey:@"DoctorDiscountedFee"]objectAtIndex:0];
                ...
                NSLog(@"%@",DocafterDisFee);
                [[hospitalTempData objectAtIndex:i] setValue:DocafterDisFee forKey:@"DocafterDisFee"];
                NSString *Distance=[[hospitalArr valueForKey:@"Distance"]objectAtIndex:0];
                [[hospitalTempData objectAtIndex:i] setValue:Distance forKey:@"Distance"];
            }
            ...
            hosptalDataBackUp =[hosptalData copy];
            self.LblAdPrice.text =[response objectForKey:@"Quotation"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.BtnAllTimings addTarget:self action:@selector(alltimingAction:) forControlEvents:UIControlEventTouchUpInside];
    
    
    cell.BtnHospitalProfile.tag =btnTag.integerValue;
    [cell.BtnHospitalProfile addTarget:self action:@selector(openHospitalAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (IBAction)alltimingAction:(id)sender {
    [Localytics tagEvent:@"OPDAllTimingClick"];
...
- (void) displayImage:(UIImageView*)imageView withImage:(UIImage*)image ImageCaption:(NSString*)caption  {
    [Localytics tagEvent:@"OPDDocImageClick"];
...
- (IBAction)didPressDocDetailBtn:(UIButton*)sender {
    [Localytics tagEvent:@"OPDDocProfileClick"];
    CGPoint buttonPosition = [sender convertPoint:CGPointZero toView:self.tableView];
    NSIndexPath *indexPath = [self.tableView indexPathForRowAtPoint:buttonPosition];
    if (indexPath != nil)
    {
...
- (IBAction)docProfileAction:(UIButton*)sender{
    [Localytics tagEvent:@"OPDDocProfileClick"];
    CGPoint buttonPosition = [sender convertPoint:CGPointZero toView:self.tableView];
    NSIndexPath *indexPath = [self.tableView indexPathForRowAtPoint:buttonPosition];
    if (indexPath != nil)
...
- (IBAction)openHospitalAction:(id)sender{
    [Localytics tagEvent:@"OPDHospitalProfileClick"];
    CGPoint buttonPosition = [sender convertPoint:CGPointZero toView:self.tableView];
    NSIndexPath *indexPath = [self.tableView indexPathForRowAtPoint:buttonPosition];
    if (indexPath != nil)
    {
...
- (IBAction)bookAppointmentAction:(id)sender {
    [Localytics tagEvent:@"OPDBookAppointmentClick"];
    CGPoint buttonPosition = [sender convertPoint:CGPointZero toView:self.tableView];
    NSIndexPath *indexPath = [self.tableView indexPathForRowAtPoint:buttonPosition];
...
- (IBAction)didPressAdEnquiryBtn:(id)sender {
     [Localytics tagEvent:@"OPDOpenEnquiryClick"];
...
-(void)notFindDataAlert{
    UIAlertView *alert =[ [UIAlertView alloc]initWithTitle:AppName message:[NSString stringWithFormat:@"Sorry!!! We didnot find the detail for 'OPD'" ] delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
    [alert show];
}
...
- (IBAction)SpecializationAction:(id)sender {
    [Localytics tagEvent:@"OPDSpecializationClick"];
    
    SelectSpecializationVC *SelectSpecializationVC = [self.storyboard instantiateViewControllerWithIdentifier:@"SelectSpecializationVC"];
    [self presentViewController:SelectSpecializationVC animated:YES completion:nil];
}

- (IBAction)sortAction:(id)sender {
  [Localytics tagEvent:@"OPDSortClick"];
    
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *DistanceNearet = [UIAlertAction actionWithTitle:@"Distance: Nearest" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self DistanceNearestAction];
    }];
    UIAlertAction *DistanceFarthest = [UIAlertAction actionWithTitle:@"Distance: Farthest" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self DistanceFarthestAction];
    }];
    UIAlertAction *feeLow = [UIAlertAction actionWithTitle:@"Fee: Low to High" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self FeeLowAction];
        
    }];
    UIAlertAction *feeHigh = [UIAlertAction actionWithTitle:@"Fee: High to Low" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self FeeHighAction];
    ...
    [alert addAction:DistanceNearet];
    //[alert addAction:DistanceFarthest];
    [alert addAction:feeLow];
    [alert addAction:feeHigh];
...
-(void)DistanceNearestAction{
    [Localytics tagEvent:@"OPDSortClickDistanceNearest"];
    self.SortLbl.text=@"Sort";
    [self.SortBtn setTitle:@"Distance: Nearest" forState:UIControlStateNormal];
    ...
    NSArray *categoryDic = [NSMutableArray arrayWithArray:[hosptalData sortedArrayUsingDescriptors:[NSArray arrayWithObject:aSortDescriptor]]];
    [hosptalData removeAllObjects];
    for (NSArray *temp in categoryDic) {
...
-(void)DistanceFarthestAction{
    [Localytics tagEvent:@"OPDSortClickDistanceFarthest"];
    self.SortLbl.text=@"Sort";
    [self.SortBtn setTitle:@"Distance: Farthest" forState:UIControlStateNormal];
    NSSortDescriptor *aSortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"Distance" ascending:YES comparator:^(id obj1, id obj2) {
...
-(void)FeeLowAction{
    [Localytics tagEvent:@"OPDSortClickFeeLowToHigh"];
    self.SortLbl.text=@"Sort";
    [self.SortBtn setTitle:@"Fee: Low to High" forState:UIControlStateNormal];
    
    NSSortDescriptor *aSortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"DocafterDisFee" ascending:YES comparator:^(id obj1, id obj2) {
...
-(void)FeeHighAction{
       [Localytics tagEvent:@"OPDSortClickFeeHighToLow"];
    self.SortLbl.text=@"Sort";
     [self.SortBtn setTitle:@"Fee: High to Low" forState:UIControlStateNormal];
    NSSortDescriptor *aSortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"DocafterDisFee" ascending:YES comparator:^(id obj1, id obj2) {
    ...
    // [hosptalData removeAllObjects];
    // [hosptalData addObject:categoryDic];
    [self.tableView reloadData];
    //DoctorFee
    
}
-(void)ResetAction{
       [Localytics tagEvent:@"OPDSortClickReset"];
    self.SortLbl.text=@"";
    [self.SortBtn setTitle:@"Sort" forState:UIControlStateNormal];
     hosptalData =[hosptalDataBackUp copy];
    [self.tableView reloadData];
}
